...
var hideDiv = function (type) {
    if (type == "1") {
        if (hideCheck == 0) {
            $("#priceRange").hide();
            $("#priceHideImg").attr("src", "/Content/images/bt_open.gif");
            hideCheck = 1;
            foldY = 450;
            var windowY = $(this).scrollTop();
            $("#GoDetailWingBanner").css({ "top": windowY + 450 })
        }
        else {
            $("#priceRange").show();
            $("#priceHideImg").attr("src", "/Content/images/bt_close.gif");
            hideCheck = 0;
            foldY = 630;
            var windowY = $(this).scrollTop();
            $("#GoDetailWingBanner").css({ "top": windowY + 630 })
        }
    }
    else if (type == "2") {
        if (hideCheck2 == 0) {
            $("#goDetail").hide();
            $("#detailHideImg").attr("src", "/Content/images/bt_open.gif");
            hideCheck2 = 1;
        }
        else {
            $("#goDetail").show();
            $("#detailHideImg").attr("src", "/Content/images/bt_close.gif");
            hideCheck2 = 0;
        }
    }
}
...
$(function () {
    ...
    //    var $PlanWingBanner = $("#PlanWingBanner");
    //    var $GoDetailWingBanner = $("#GoDetailWingBanner");
    //   
    //    var $gnbStco = $("#gnb_stco"), $gnbStcoBg = $gnbStco.parent("div");
    //    $(".header_search_input").each(function (index, item) {
    //        var $headerSeachVal = $(item).val();
    //        $(item).focus(function () {
    //            var val = $(item).val();
    //            if (val == $headerSeachVal) $(item).val("");
    //        }).blur(function () {
    //            var val = $(item).val();
    //            if (val == "") $(item).val($headerSeachVal);
    //        });
    //    });
    //    $gnbStco.find(".hover").hover(function () {
    //        if ($(this).is(".li1")) $gnbStcoBg.addClass("all");
    //        else $gnbStcoBg.addClass("on");
    //    }, function () {
    //        if ($(this).is(".li1")) $gnbStcoBg.removeClass("all");
    //        else $gnbStcoBg.removeClass("on");
    //    });
    ...
    //            section1OffSet = section1OffSet - 100;
    //        }



    //    } catch (e) { section1OffSet = 300; }
    //    try {
    ...
    $(window).scroll(function () {
        ...
        //        alert(floatingImg.naturalWidth);
        //        alert(floatingImg.naturalHeight);

        var lo = location.href.toLowerCase();
        var chrome_top = 0;
        var explore_top = 0;

        if (lo.indexOf("productplan") > -1) {
            chrome_top = 860;
            explore_top = 830;
        }
        else {
        ...
        if (typeof floatingImg != "undefined") {
            if (agent.indexOf("chrome") != -1) {
                $floatingBanner.css({ "top": chrome_top - floatingImg.naturalHeight });
                $floatingBanner.css({ "margin-left": 1030 - floatingImg.naturalWidth });
                $(".floatingimg").css({ "left": floatingImg.naturalWidth - 40 });

            } else {
                $floatingBanner.css({ "top": explore_top - floatingImg.naturalHeight });
                $floatingBanner.css({ "margin-left": 1030 - floatingImg.naturalWidth });
                $(".floatingimg").css({ "left": floatingImg.naturalWidth });
                $("#floatingXbtn").css({ right: 5 });
            }
        }

        $floatingBanner.css({ "position": "fixed" });
...
function closeTopBanner() {
    ...
    countTop = 0;
    //위치 변경처리
    var height = parseInt($(".top_banner_wrap").css("height").replace("px"));
...
function noMoreView() {
    $("#floatingBanner").hide();
    setCookieHour("floatingCookie", "1", 6);
    $("#smallTodayPrice").show();
}

function setCookieHour(cName, cValue, cHour) {
    var expire = new Date();
...
//트래킹 로그 등록
function fn_TrackingLog_Ins(moveurl, target) {
    
    $.ajax({
        type: "POST",
        url: "/Shared/PageTrackingLOG",
        data: { url: window.location.href, moveurl: moveurl },
        dataType: "json",
        success: function (data) {
            console.log("tracking-log:" + data);
        },
        error: function (xhr, status, error) {
            //console.log("tracking-log-error:" + xhr.responseText);
        }
    });

    if (moveurl != "") {
        if (target == "_blank") {
            window.open(moveurl, target);
        } else {
